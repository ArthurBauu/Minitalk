...
static int	is_a_space(char c, char const *sep)
{
	size_t	i;
...
static void	copystr(const char *s, char *tempstr,
...
char	*ft_strtrim(const char *s, char const *set)
{
	...
	if (!s)
		return (NULL);
	i = 0;
	while (is_a_space(s[i], set))
		i++;
	if (s[i] == '\0')
		return (ft_strdup(""));
	j = (unsigned int)(ft_strlen(s) - 1);
	while (j > i && is_a_space(s[j], set))
		j--;
	tempstr = malloc(sizeof(char) * (j - i + 2));
	if (!tempstr)
		return (NULL);
	copystr(s, tempstr, i, j);
	return (tempstr);
}
